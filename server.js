...
// Book.find({}).where('pages').gt(200).lt(500).exec().then(result => console.log(result));

//2. Find books whose rating is less than 5, and sort by the author's name

// Book.find({}).where('rating').lt(5).sort('-author').exec().then(result => console.log(result));

//3. Find all the Fiction books, skip the first 2, and display only 3 of them

// Book.find({ genres: 'Fiction' }).skip(2).limit(3).exec().then(result => console.log(result));
...
// Person.find({}).and([{ height: { $gt: 180 } }, { salary: { $gt: 30000 } }]).exec().then(result => console.log(result));

//2. Find all the people who are tall (>180) OR rich (>30000)

// Person.find({}).or([{ height: { $gt: 180 } }, { salary: { $gt: 30000 } }]).exec().then(result => console.log(result));
...
// Person.find({}).where("kids").elemMatch({ hair: "grey" }).exec().then(result => console.log(result))
